...
QUnit.test("ЗАДАЧА 14. X4(x1, y1, x2, y2, x3, y3) и Y4(x1, y1, x2, y2, x3, y3)", 
  function(assert){
	...
	assert.deepEqual(X4(0, 0, 3, -3, 1, -5), -2, "Не выполнено X4(x1, y1, x2, y2, x3, y3) === -2"); //прямоугольник, стоящий под углом
	assert.deepEqual(Y4(0, 0, 3, -3, 1, -5), -2, "Не выполнено X4(x1, y1, x2, y2, x3, y3) === -2");

	assert.deepEqual(X4(0, 0, 8, 0, 0, -3), 8, "Не выполнено X4(x1, y1, x2, y2, x3, y3) === 8"); //координатные точки, не идущие последовательно
